...
import { FileExtIcon } from "@/components/agent-elements/icons/file-ext-icon";
import {
  mapToolInvocationToStep,
  mapToolStateToStepState,
} from "@/components/agent-elements/utils/tool-adapters";
...
export function EditToolDiffCard({
  ...
  const diffFiles = React.useMemo(() => {
    ...
    const fallbackNew = step.diffLines
      ?.filter((line: any) => line.type !== "remove")
      .map((line: any) => line.content)
      .join("\n");
      ...
      >
        ...
        {step.diffStats && !isPending && (
          <span className="text-[11px] font-mono text-an-tool-color-muted inline-flex gap-2">
            {step.diffStats.split(" ").map((token: string) => (
              <span
                key={token}
                className={
                  token.startsWith("+")
