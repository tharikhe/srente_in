...
import { getToolStatus } from "@/components/agent-elements/utils/format-tool";
...
import { QuestionTool } from "@/components/agent-elements/question/question-tool";
...
export const ToolRenderer = memo(function ToolRenderer({
  ...
  // Custom tool renderer passed via props
  const partName = partType.replace(/^tool-/, "");
  if (toolRenderers?.[partName]) {
    const CustomRenderer = toolRenderers[partName];
    return (
      <CustomRenderer
        name={partName}
